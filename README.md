...
## 制作图片列表区域
1. 图片列表需要懒加载技术 可以使用 Mint-UI 提供的组件 'lazy-load'
2. 渲染图片列表
...
3. 注意：
 + img标签上的class不能去掉
 + 每个图片数据对象中 必须要有 width 和 height 属性

## 绘制商品列表页面基本结构

## 如何在手机端 进行项目的预览与测试
1. 首先确保 手机和开发项目所用的电脑处在同一 WIFI 环境中 即手机可以访问到电脑的IP
2. 在项目文件 package.json 中 在dev脚本中 添加一个 --host 指令 并把当前电脑的 WIFI IP 地址 设置为 --host 的指令值
3. 如何查看自己电脑所处 WIFI 的 IP：
 + 在cmd终端 键入 'ipconfig' 命令 查看无线网的ip地址
